...
template<typename T>
Matrix<T>::Matrix(std::vector<std::vector<T>> c) {
    ...
    for (int i = 0; i < content.size(); i++) {
        ...
        for_each(content[i].begin(),
                 ...
                 [&forPrint, &maxLen, *this](T &n){
                    for (int j = 0; j < (maxLen - std::to_string(n).length()); j++) {
                        forPrint += " ";
                    }
...
Matrix<T>& Matrix<T>::operator*=(Matrix<T> rhs) {
    try {
        if (this->numCols() != rhs.numRows()) { throw 505; }
        else {
            std::vector<std::vector<T>> newContent = {};
...
Matrix<T>& Matrix<T>::operator+=(Matrix<T> rhs) {
    try {
        if (this->numCols() != rhs.numCols() || this->numRows() != rhs.numRows()) {
            throw 505;
        }
        else {
            for (int row = 0; row < this->numRows(); row++) {
                for (int col = 0; col < this->numCols(); col++) {
                    this->setVal(row, col, this->getVal(row, col) + rhs.getVal(row, col));
                }
            }
        }
    } catch (...) {
...
Matrix<T> Matrix<T>::gramSchmidt(bool isOrthonormal) {
    ...
    for (int i = 1; i < this->numCols(); i++) {
        ...
        for (int j = 0; j < i; j++) {
            std::vector<T> proj = projection(transposed.content[i], qContent[j]);
...
template <typename T>
std::vector<Matrix<T>> Matrix<T>::QRFactorization() {
