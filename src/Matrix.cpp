...
template <typename T> Matrix<T> &Matrix<T>::operator*=(double scalar) {
...
template <typename T> Matrix<T> operator*(Matrix<T> lhs, double scalar) {
    return lhs *= scalar;
}
// For commutativity
template <typename T> Matrix<T> operator*(double scalar, Matrix<T> rhs) {
    return rhs *= scalar;
}
